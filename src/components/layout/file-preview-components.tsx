...
export function renderFilePreview(
  ...
  if (isPdfFile(path)) {
    return file.documentPreview ? (
      <DocumentPreview file={file} project={project} preview={file.documentPreview} />
    ) : file.previewDataUrl ? (
      <PdfPreviewFrame file={file} project={project} />
    ) : (
      <BinaryPreviewFallback file={file} project={project} />
    );
  }
  ...
  return <MarkdownFilePreview content={content} />;
}

const markdownPreviewComponents: Components = {
  table: ({ children }) => (
    <div className="markdown-preview-table-wrap">
...
function MarkdownFilePreview(props: { content: string }): JSX.Element {
  return (
    <div className="markdown-preview">
      <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownPreviewComponents}>
        {normalizeMarkdownPreviewInput(props.content)}
      </ReactMarkdown>
    </div>
  );
}

function normalizeMarkdownPreviewInput(content: string): string {
  ...
  for (const line of lines) {
    ...
    if (
      !inFence &&
      isStandaloneThematicBreak(line) &&
      normalized.length > 0 &&
      ...
      <div className="pdf-preview-toolbar">
        ...
        <div className="pptx-preview-actions">
          <Button
            size="sm"
            variant="secondary"
            leadingIcon={<ExternalLink size={13} aria-hidden="true" />}
            onClick={() => openProjectFile(props.project, props.file)}
          >
          ...
          <Button
            size="sm"
            variant="secondary"
            leadingIcon={<FolderOpen size={13} aria-hidden="true" />}
            onClick={() => revealProjectFile(props.project, props.file)}
          >
...
function DocumentPreview(props: {
  ...
  const previewLabel = isDocx
    ? localize(
        language,
        `DOCX 文档预览 · ${props.preview.pageCount} 段`,
        `DOCX document preview · ${props.preview.pageCount} sections`
      )
    : isPdf
      ? localize(
          language,
          `PDF 页面预览 · ${props.preview.pageCount} 页`,
          `PDF page preview · ${props.preview.pageCount} page`
        )
      : localize(
          language,
          `PPTX 幻灯片预览 · ${props.preview.pageCount} 页`,
          `PPTX slide preview · ${props.preview.pageCount} slides`
        );
  ...
  return (
    <div
      className={`pptx-preview-frame ${isDocx ? 'docx-preview-frame' : ''} ${isPdf ? 'pdf-document-preview-frame' : ''}`}
    >
      ...
      {isPdf ? (
          ...
          {props.preview.pages.map((page) => (
            <article key={page.index} className="pdf-document-page-card">
              {page.thumbnailDataUrl ? (
                <img
                  className="pdf-document-page-image"
                  src={page.thumbnailDataUrl}
                  alt={page.title || `PDF page ${page.index}`}
                />
              ) : (
        ...
        <div className="pptx-slide-grid">
          {props.preview.pages.map((page) => (
            <article
              key={page.index}
              className={`pptx-slide-card ${page.thumbnailDataUrl ? 'thumbnail' : 'text-only'}`}
            >
              <div className="pptx-slide-index">{page.index}</div>
              <div className="pptx-slide-body">
                ...
                {page.thumbnailDataUrl ? (
                  <img
                    ...
                    src={page.thumbnailDataUrl}
                    alt={page.title || `Slide ${page.index}`}
                  />
                ) : (

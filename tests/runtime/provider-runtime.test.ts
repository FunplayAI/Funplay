...
test('runtime capabilities expose native boundaries', () => {
...
test('runtime resolver always resolves the native runtime', () => {
  ...
  assert.equal(resolveGenericAgentRuntime({ runtimeId: 'native', provider: anthropicProvider }).id, 'native');
  ...
  assert.equal(resolveGenericAgentRuntime({ provider: anthropicProvider, runtimeStrategy: 'auto' }).id, 'native');
});

...
test('stream manager tracks parallel sessions within and across projects', () => {
  ...
  assert.deepEqual(getStreamSessionForSession('project_a', 'session_2')?.pendingPermission?.impact?.paths, ['src/app.ts']);
...
test('stream manager keeps separate tool activities at the same text boundary', () => {
  ...
  const current = getStreamSessionForSession('project_a', 'session_1');
  assert.equal(current?.activityItems.length, 2);
  assert.deepEqual(current?.activityItems.map((activity) => activity.toolUseIds), [['tool_read_a'], ['tool_read_b']]);
  assert.deepEqual(current?.activityItems.map((activity) => activity.status), ['completed', 'completed']);
  assert.deepEqual(current?.activityItems.map((activity) => activity.offset), [
    '我会同时读取两个文件。\n\n'.length,
    '我会同时读取两个文件。\n\n'.length
  ]);
});

...
const projectSessionRuntimeIdSchema = z.custom<ProjectSessionRuntimeId>((value) => ['native'].includes(String(value)));
const projectSessionEffortSchema = z.custom<ProjectSessionEffort>((value) =>
...
const mcpToolPolicyOverrideSchema = z
  .object({
...
const providerChunkTimeoutSchema = z
  .number()
  .int()
  .min(1)
  .max(60 * 60 * 1000)
...
export const environmentActionInputSchema = environmentInputSchema
  .extend({
    actionId: environmentActionKindSchema
  })
  .strict();

export const folderPickerInputSchema = z
  .object({
    mode: projectSetupModeSchema,
...
export const mcpPluginInputSchema = z
  .object({
    name: trimmedString(1, 120),
...
export const updateAgentSettingsSchema = z
  ...
  .strict()
  .refine((input) => Object.keys(input).length > 0, 'At least one agent setting must be provided.');
...
export const updateProjectAgentPolicySchema = z
  ...
  .strict()
  .refine((input) => Object.keys(input).length > 0, 'At least one project agent policy setting must be provided.');

export const listAgentSkillCatalogSchema = z
  .object({
...
export const updateSessionRuntimeSchema = z
  .object({
  ...
  .strict()
  .refine((input) => Object.keys(input).length > 0, 'At least one session runtime override must be provided.');
...
export const assetGenerationProviderInputSchema = z
  .object({
    name: trimmedString(1, 120),
...
export const runtimeDoctorInputSchema = z
  .object({
    providerId: optionalTrimmedString(120),
    projectId: optionalTrimmedString(120),
  ...
  .strict()
  .default({});
export const runtimeRepairInputSchema = z
  .object({
...
export const agentUserInputResponseSchema = z
  .object({
...
export const promptAttachmentImportItemSchema = z
  .object({
  ...
  .strict()
  .refine(
    (value) => Boolean(value.path || value.dataUrl),
    'Attachment import item requires path or dataUrl'
  ) satisfies z.ZodType<PromptAttachmentImportItem>;
export const promptAttachmentImportItemsSchema = z.array(promptAttachmentImportItemSchema).max(12).default([]);
export const filePathSchema = trimmedString(1, 2048).refine(

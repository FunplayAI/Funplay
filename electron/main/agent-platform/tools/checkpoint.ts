...
registerAgentTool({
  ...
  description:
    '查看当前 Agent 运行 checkpoint 以来被 Funplay 写入工具记录的文件变更和 diff 预览；二进制文件只显示字节大小变化，不输出文本 diff；不会写入文件。',
...
registerAgentTool({
  ...
  description:
    '将当前 Agent 运行 checkpoint 记录过的文件恢复到运行前状态（二进制文件按原始字节恢复）。超过大小上限未纳入 checkpoint 的文件不会被回滚。适合用户要求撤回本轮文件修改；高风险，host 会在执行点做权限判断。',
  inputSchema: z.object({

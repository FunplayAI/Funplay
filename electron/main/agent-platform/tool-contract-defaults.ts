...
const toolContractSpecs = {
  ...
  run_engine_environment_action: { label: 'Run engine environment action', activity: 'Running engine environment action', family: 'engine', usageHint: 'Drive a staged engine setup action (create/import/open project, install bridge, verify path); not for installing engine software itself.', target: targetFrom('actionId', 'projectPath', 'platform'), sideEffect: (input) => activeEngineSideEffect('run_engine_environment_action', input), isConcurrencySafe: neverSafe },

...
function buildProviderProbe(state: AppState, options: RuntimeDoctorOptions, provider?: AiProvider): RuntimeDoctorProbe {
  ...
  if (defaultProviderMissing) {
    findings.push({
      ...
      recoveryActions: provider
        ? [{ label: '设为默认 Provider', command: 'providers:repair set-default-provider' }]
        : undefined
    });
  }
  ...
  const looksOfficialAnthropic =
    providerMarker.includes('api.anthropic.com') || providerMarker.trim() === 'anthropic anthropic';
  if (
    provider &&
    provider.protocol === 'anthropic' &&
    resolved.authStyle !== 'env_only' &&
    !resolved.baseUrl &&
    !looksOfficialAnthropic
  ) {
    findings.push(
      providerFinding(provider, {
        severity: 'warn',
  ...
  if (resolved.authStyle === 'api_key' || resolved.authStyle === 'auth_token') {
    findings.push(
      providerFinding(provider, {
        ...
        suggestedAction: hasSecret ? undefined : '在 Provider 设置中保存 API key/token。'
      })
    );
    ...
    findings.push(
      providerFinding(provider, {
        ...
        suggestedAction: credentialHints.length > 0 ? undefined : '确认 AWS/Google 等所需环境变量或本机凭据已配置。'
      })
    );
  }
...
function buildFeatureProbe(provider?: AiProvider): RuntimeDoctorProbe {
  ...
  const model = provider?.availableModels?.find(
    (item) => item.modelId === provider.model || item.upstreamModelId === provider.upstreamModel
  );
  ...
  if (provider.name.toLowerCase().includes('custom') || !provider.baseUrl.trim()) {
    findings.push(
      providerFinding(provider, {
        ...
        code: provider.baseUrl.trim()
          ? 'native_custom_provider_needs_live_probe'
          : 'native_custom_provider_base_url_missing',
...
async function buildNetworkProbe(provider?: AiProvider, live?: boolean): Promise<RuntimeDoctorProbe> {
  ...
  try {
    ...
    const severity: RuntimeDoctorFinding['severity'] =
      response.status >= 500 || response.status === 429 ? 'warn' : 'ok';
    return createProbe('network', 'Network/provider', startedAt, [
      providerFinding(provider, {
        severity,
        ...
        suggestedAction: response.status === 429 ? '稍后重试，或检查服务商额度/限速。' : undefined
      })
    ]);
  } catch (error) {
    return createProbe('network', 'Network/provider', startedAt, [
...
export function repairRuntimeDoctor(
  state: AppState,
  input: RuntimeRepairInput
): { success: true; stateChanged: boolean } {
  switch (input.actionId) {
    ...
    case 'switch-auth-style': {
      const providerId = input.providerId?.trim();
      const authStyle =
        input.authStyle ?? (input.actionId === 'switch-auth-style-auth-token' ? 'auth_token' : 'api_key');
